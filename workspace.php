...
function workspace_ai_code($instruction, $contextFiles, $repoFiles, $user, &$rawReply, &$error) {
    ...
    ], defined('CODE_AGENT_MODEL') ? CODE_AGENT_MODEL : 'codestral-latest', [
        'temperature' => 0.25,
        'max_tokens' => 8192,
    ]);
...
.review-code,.review-diff{height:100%;min-height:360px;overflow:auto;padding:0;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:12.5px;line-height:1.55;color:#e5e1ec;background:#0b0b12}
.code-table{min-width:100%;border-collapse:collapse}
.code-table tr{border-bottom:1px solid rgba(255,255,255,.025)}
.code-table td{vertical-align:top}
.code-ln{width:46px;padding:0 10px;text-align:right;color:#5f5a70;background:#09090f;user-select:none;border-right:1px solid var(--line)}
.code-src{padding:0 14px;white-space:pre}
.diff-line{display:grid;grid-template-columns:46px 1fr;min-width:max-content;border-bottom:1px solid rgba(255,255,255,.025)}
.diff-ln{padding:0 10px;text-align:right;color:#5f5a70;background:#09090f;user-select:none;border-right:1px solid var(--line)}
.diff-src{padding:0 14px;white-space:pre}
.diff-line.add{background:rgba(74,222,128,.09)}
.diff-line.add .diff-src{color:#c8f7d8}
.diff-line.meta{background:rgba(230,18,42,.12)}
.diff-line.meta .diff-src{color:#ffb7c0;font-weight:700}
.diff-line.ctx .diff-src{color:#a8a3b7}
.tok-kw{color:#ff7b9c}.tok-str{color:#ffd166}.tok-com{color:#7d768d}.tok-fn{color:#8bd3ff}.tok-num{color:#c3a6ff}
...
function formatGeneratedDiff(file) {
  ...
  return header.concat(lines.map(line => '+ ' + line));
}

function highlightCodeLine(line, kind) {
  let html = previewEscapeHtml(line);
  html = html.replace(/(\/\/.*$|\/\*.*?\*\/)/g, '<span class="tok-com">$1</span>');
  html = html.replace(/(['"`])((?:\\.|(?!\1).)*)\1/g, '<span class="tok-str">$1$2$1</span>');
  html = html.replace(/\b(import|from|export|default|function|const|let|var|return|if|else|for|while|class|extends|new|async|await|try|catch|true|false|null|undefined)\b/g, '<span class="tok-kw">$1</span>');
...
function renderCodeViewer(file) {
  ...
  const lines = String(file.content || '').split('\n');
  return '<table class="code-table" aria-label="Code généré">' + lines.map((line, index) => (
    '<tr><td class="code-ln">' + (index + 1) + '</td><td class="code-src">' + highlightCodeLine(line, kind) + '</td></tr>'
  )).join('') + '</table>';
}

function renderDiffViewer(file) {
  const lines = formatGeneratedDiff(file);
  return lines.map((line, index) => {
    const cls = line.startsWith('+ ') ? 'add' : (line.startsWith('diff ') || line.startsWith('---') || line.startsWith('+++') || line.startsWith('@@') ? 'meta' : 'ctx');
    const shown = line.startsWith('+ ') ? line.slice(2) : line;
    return '<div class="diff-line ' + cls + '"><span class="diff-ln">' + (index + 1) + '</span><span class="diff-src">' + highlightCodeLine(shown, fileKind(file.path)) + '</span></div>';
  }).join('');
}
...
function renderGeneratedReview() {
  ...
  if (!file) {
    code.innerHTML = '<div class="review-empty">Aucun fichier sélectionné.</div>';
    diff.innerHTML = '<div class="review-empty">Aucun fichier sélectionné.</div>';
    frame.srcdoc = '';
  ...
  code.innerHTML = renderCodeViewer(file);
  diff.innerHTML = renderDiffViewer(file);
