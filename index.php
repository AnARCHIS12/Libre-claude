    ...
    <div class="input-inner">
      ...
      <div class="model-bar">
        ...
        <button type="button" class="model-picker-btn" id="model-picker-btn" onclick="toggleModelMenu()" aria-haspopup="listbox" aria-expanded="false">
          <span id="model-picker-label"><?= htmlspecialchars($modelNames[$defaultModel] ?? 'Claude Sonnet 5') ?></span>
...
function isImagePrompt(text) {
  ...
  // Jamais une image si c'est une question ou une tâche de texte
  if (/\b(comment|pourquoi|how|why|qu est ce que|qu est-ce|c est quoi|kezako)\b/.test(s)) return false;
  const asksForNonImage = /\b(explique|expliquer|analyse|analyser|resume|resumer|recherche|rechercher|cherche|chercher|code|coder|script|programme|fonction|classe|ecris|ecrire|redige|rediger|traduis|traduire|corrige|corriger|histoire|poeme|article|paragraphe|lettre|mail|email|site|web|app|application|calcule|calcule|liste|donne moi une liste|donne une liste)\b/.test(s);
  ...
  // 1. Verbe de création + mot clé image explicite
  ...
  // 2. Mot image + "de / d / pour"
  if (new RegExp(`\\b${imageWords}\\s+(de|d |pour)\\b`).test(s)) {
    if (!asksForNonImage) return true;
  }

  // 3. Verbes de dessin / peinture directs
  if (/\b(dessine|dessines|dessinez|dessiner|illustre|illustres|illustrez|illustrer|peins|peint|peindre|peignez|croque|croquer|draw|draws|drawing|paint|painting)\b/.test(s)) {
    if (!asksForNonImage) return true;
  }

  // 4. Verbe de création + (pronom optionnel) + article + N'IMPORTE QUEL NOM
  // → ultra-permissif : "génère une femme blonde", "fais un truc stylé", "crée un machin"
  const creationVerb = '\\b(genere|generer|generez|cree|creer|creez|fais|fait|faire|make|create|dessine|illustre|peins)\\b';
  const pronounOrNot = '(\\s+(moi|nous|me|us))?';
  const article = '\\s+(un|une|des|le|la|les|a|an|the|du)';
  // Le nom peut être ANYTHING (au moins 2 chars, pas juste un mot fonctionnel)
  if (new RegExp(`${creationVerb}${pronounOrNot}${article}\\s+\\w{2,}`).test(s)) {
    if (!asksForNonImage) return true;
  }

  // 5. Entrée courte (≤10 mots) avec un sujet visuel reconnaissable
  const words = s.split(/\s+/).filter(Boolean);
  const visualNoun = /\b(chat|chaton|chien|chiot|animal|animaux|personnage|portrait|paysage|scene|robot|voiture|maison|ville|chateau|arbre|fleur|foret|montagne|mer|ocean|plage|ordinateur|chambre|bureau|bateau|avion|fusee|galaxie|planete|etoile|soleil|lune|monstre|dragon|cheval|oiseau|poisson|lion|tigre|loup|renard|ours|panda|singe|mouton|femme|homme|fille|garcon|personne|enfant|bebe|guerrier|chevalier|ninja|samourai|elfe|vampire|zombie|alien|soldat|heroine|super.?hero)\b/.test(s);
  const visualCue = /\b(rouge|noir|noire|bleu|bleue|vert|verte|jaune|rose|violet|orange|blanc|blanche|blonde|blond|brune|brun|rousse|roux|grand|grande|petit|petite|jeune|vieux|belle|beau|punk|cyberpunk|steampunk|realiste|stylise|style|minimaliste|3d|anime|manga|vectoriel|cinematique|futuriste|sombre|lumineux|retro|vintage|neon|pixel)\b/.test(s);
  if (words.length <= 10 && !asksForNonImage && visualNoun && visualCue) return true;
